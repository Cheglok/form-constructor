...
export function validateFormBuilder({ newFieldData, sourceType }: { newFieldData: FormField; sourceType: string }): {
    isValid: boolean;
    ...
    if (validatedField.tag !== FieldTag.UI_BUTTON) {
        ...
        if (sourceType === DataSourceType.API) {
            ...
            if (validatedField.tag === FieldTag.UI_SELECT) {
                ...
                if (validatedField.currentValue) {
                    const isInitialValueCorrect = validatedField.options.some(
                        (option: string) => option === validatedField.currentValue
                    );
